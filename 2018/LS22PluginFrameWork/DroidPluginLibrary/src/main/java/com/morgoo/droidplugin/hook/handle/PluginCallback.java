...
public class PluginCallback implements Handler.Callback {
    ...
    public boolean handleMessage(Message msg) {
        ...
        try {
            ...
            android.util.Log.e("TAG", "PluginCallback handleMessage:");
    ...
    //            return handleServiceArgs(msg);
            ...
            if (mCallback != null) {
                android.util.Log.e("TAG", "PluginCallback handleMessage mCallback:");
        ...
        try {
            ...
            if (targetIntent != null && !isShortcutProxyActivity(stubIntent)) {
                ...
                if (targetActivityInfo != null) {
                    ...
                    //如果stubActivityInfo不为null,就上报这个冒充的intent给当前进程，这样就相当于获得了一张身份通行证。从此就无敌了，下面这个方法相当于
                    //身份通过后，把它带来的消息向上传，殊不知，这个是冒充的
                    ...
                    //追中preLoadApk方法，发现里面 classloader = new PluginClassLoader(apk,
                    // optimizedDirectory, libraryPath, hostContext.getClassLoader().getParent())
                    // ;这个PluginClassLoader是继承DexClassLoader，注释上是
